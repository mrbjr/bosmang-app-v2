...
class BosmangCore extends Component {
    ...
    elementRo = (element) => {
        var sigma_s = element.sigma
        console.log("S")
        console.log(sigma_s)
        var r = freq.map((freq,index)=>{return (Math.PI*element.fc*element.sigma[index])/(4*freq*element.ntotlab[index])})
        console.log("R")
        console.log(r)
        var sigma_a = r.map ((r,index)=>{return (element.sigmaf[index]+r*element.sigma[index])/(1+r)})
        console.log("A")
        console.log(sigma_a)

        return r.map((r,index)=>{return element.Rlab[index]+Math.min(8,10*log10(sigma_a[index]/sigma_s[index]))})
    }

    elementLn = (element,ts) =>{return freq.map((freq,index)=>{
...
//----------------------------------------------------------------------------------------------------------------------------//
 ...
 //----------------------------------------------------------------------------------------------------------------------------//       
    ...
    floatf0 = (element) => {return 160*Math.sqrt(element.ym/element.smass) || 10000000000}

    floatd = (element) => { switch (element.tp){
        case 'sand':
        return freq.map(freq => {return Math.max(30*log10(freq/element.f0),0)})
        
        case 'asphalt':
        return freq.map(freq => {return Math.max(40*log10(freq/element.f0),0)})
        }
    }
    ...
    predictionDnT = (R) => {
        return R.map((R,index) => {return R + 10*log10((0.032*this.props.x*this.props.y*this.props.z2)/(this.props.x*this.props.y))}) // Eq 3 (pt 2)
    }
    ...
    render(){
        ...
        floor.Lnsitu = this.elementLn(floor,floor.Tssitu)
        ...
        //flank_00.Lnsitu = this.state.floor.Lnsitu
        //flank_00.dLsitu = this.state.float.d
        //flank_00.Lnd = this.flankLnd(flank_00)
        ...
        prediction.DnT = this.predictionDnT(prediction.R)
        prediction.LnT = this.predictionLnt(prediction.L)
        prediction.Lnw = tabajara(prediction.L,"Ln,w")
        ...
        return (
            <div>
                ...
                <div class="div_results_container">
                    <div class="div_results">
                        <span class="spam_results_header"> L<sub>n,w</sub> (C<sub>I</sub>; C<sub>I,50-2500</sub>): </span> <br></br>
                        <span class="span_results"> {floor.Llabw[0].toFixed(1)} ({floor.Llabw[1]}; {floor.Llabw[2]}) dB </span>
                    </div>
                    <div class="div_results">
                        <span class="spam_results_header"> L'<sub>n,w</sub> (C<sub>I</sub>; C<sub>I,50-2500</sub>): </span> <br></br>
                        <span class="span_results"> {prediction.Lnw[0].toFixed(1)} ({prediction.Lnw[1]}; {prediction.Lnw[2]}) dB </span>
                    </div>
                    <div class="div_results">
                ...
                <div class="div_results_table_content">
                    <table class="result_table_flank">
                        <thead class="result_thead">
                            <tr>
                                <th rowSpan="2">F</th>
                                <th class="result_thead_small">Direto  </th>
...
class FlankValueList extends Component{
        ...
        const rows = freq.map((row, index) => {
            return (
              <tr key={index} name={index}>
                ...
                <td class="table_row result_row">{flank_floor_floor.Lnd[index].toFixed(1)}</td>
                <td class="table_row result_row">{flank_a.type == "T" ? flank_a.Lnijsitu[index][2].toFixed(1) : flank_a.Lnijsitu[index][0].toFixed(1)}</td>
                <td class="table_row result_row">{flank_b.type == "T" ? flank_b.Lnijsitu[index][2].toFixed(1) : flank_b.Lnijsitu[index][0].toFixed(1)}</td>
                <td class="table_row result_row">{flank_c.type == "T" ? flank_c.Lnijsitu[index][2].toFixed(1) : flank_c.Lnijsitu[index][0].toFixed(1)}</td>
                <td class="table_row result_row">{flank_d.type == "T" ? flank_d.Lnijsitu[index][2].toFixed(1) : flank_d.Lnijsitu[index][0].toFixed(1)}</td>
                <td class="table_row result_row">{prediction.L[index].toFixed(1)}</td>
                <td class="table_row result_row">{prediction.LnT[index].toFixed(1)}</td>
              </tr>
            )
          })
